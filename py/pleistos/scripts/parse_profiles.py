        ...
        with open(path, "r") as file:
            soup = BeautifulSoup(
                ...
                parse_only=SoupStrainer(
                    name="div", attrs={"id": "ProfilePleistocene", "class": "section"}
                ),
            )
            file.close()
            ...
            if search("^-*\d+", elevations[0]) is not None:
                summit_elevations_meters.append(str(search("^-*\d+", elevations[0])[0]))
                summit_elevations_feet.append(str(search("^-*\d+", elevations[1])[0]))
            else:
                summit_elevations_meters.append(str(elevations[0]))
                summit_elevations_feet.append(str(elevations[0]))
...
if __name__ == "__main__":
    ...
    # seed("Isekaijoucho")
    # test_paths = choices(paths, k=521)
